package app.cicilan.presentation.settings
...
import app.cicilan.customview.preference.CustomPreference
import app.cicilan.presentation.BaseFragment
import app.cicilan.presentation.R
import app.cicilan.presentation.SettingsViewModel
import app.cicilan.presentation.databinding.DialogSettingsLanguageBinding
import app.cicilan.presentation.databinding.DialogSettingsThemeBinding
import app.cicilan.presentation.databinding.MainSettingsBinding
import app.cicilan.util.dotPixel
import app.cicilan.util.runWhenResumed
...
class MenuSettingsFragment : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<CustomPreference>(getString(R.string.key_theme))?.apply {
            runWhenStarted {
                /* val theme = when (viewModel.getThemeValue.first()) {
        ...
        findPreference<CustomPreference>(getString(R.string.key_language))?.apply {
            runWhenStarted {
                /*val language = when (viewModel.getLangValue.first()) {
        ...
        findPreference<Preference>(getString(R.string.key_feedback))?.setOnPreferenceClickListener {
            ...
            startActivity(
                Intent(Intent.ACTION_SENDTO).setData(Uri.parse("mailto:$email?subject=$subject")),
            )
            true
        }
        ...
        findPreference<Preference>(getString(R.string.key_rating))?.setOnPreferenceClickListener {
            startActivity(
                Intent(Intent.ACTION_VIEW)
                    .setData(Uri.parse("https://play.google.com/store/apps/details?id=app.cicilan"))
                    .setPackage("com.android.vending"),
            )
            true
        }

        findPreference<Preference>(getString(R.string.key_donate))?.setOnPreferenceClickListener {
            findNavController().navigate(R.id.action_settings_to_donate)
            true
        }

        findPreference<Preference>(getString(R.string.key_privacy))?.setOnPreferenceClickListener {
            showMessage(getString(R.string.otw_desc))
            true
        }

        findPreference<Preference>(getString(R.string.key_faq))?.setOnPreferenceClickListener {
            showMessage(getString(R.string.otw_desc))
            true
        }

        findPreference<CustomPreference>(getString(R.string.key_about))?.apply {
